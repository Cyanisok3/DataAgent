...
When you finish, make the best valid implementation you can and run the most relevant dbt validation. Your final response must contain exactly one relative path to a real `.duckdb` file in the workspace, or the exact text `NO_ARTIFACT` when no submit-able artifact exists. Do not submit SQL, CSV, directories, multiple files, or an explanation around the final path.
